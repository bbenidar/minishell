...
/*   Updated: 2023/07/31 06:27:58 by sakarkal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_first_check(char *str)
{
	if (str)
	{
		...
		if (ft_check_quotes(str))
			...
			printf("\033[0;31mminishell: syntax error \
				near unexpected token `| or &`\033[0m\n");
		else if (check_logical(str))
			printf("\033[0;31mminishell: LOGICAL ERROR \
				(logic operation not handled)\033[0m\n\n");
		else if (check_redir(str))
			printf("\033[0;31mminishell: syntax error \
				near unexpected token `<'\033[0m\n\n");
		else
			return (0);
	}
	return (1);
